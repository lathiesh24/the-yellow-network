...
const WebSingleIndustry = ({ onDotClick, selectedSector }) => {
  const radius = 315;
  const centerX = 1515;
  const centerY = 338;
  const numCircles = 8;
  ...
  const circleNames = selectedSectorData
    ? selectedSectorData.industries
        .map((industry) => industry.industryName)
        .slice(0, numCircles)
    : Array(numCircles).fill("No Industries Found");
...
const handleClick = (index) => {
  const selectedIndustry = circleNames[index]; 
  const targetAngle = (highlightedIndex - index) * ((2 * Math.PI) / numCircles);
  setCurrentAngle((prev) => prev + targetAngle);
  setHighlightedIndex(index);

  if (index === highlightedIndex) {
    onDotClick(selectedIndustry);
  }
};
...
export default WebSingleIndustry;
